...
func (uv * GtkUserView) InitMap (ngrid, nrole, npeople, ncity, ninst, ntrain, ntech, ncard, nx, ny int) {// 初始化地图
      init_map (ngrid, nrole, npeople, ncity, ninst, ntrain, ntech, ncard, nx, ny)
}
...
func (uv * GtkUserView)     NoteRoleMove (rind int, sloc int, oloc int, step int) {// 角色移动一个回合
      set_role (sloc, rind, 1)
      set_role (oloc, rind, 0)
...
func (uv * GtkUserView)     BattleTech (ss int, tind int, sst * mbg.BattleState, ost * mbg.BattleState) {// 战斗时发动技能. ss: 0|1 发动方
      ...
      battle_msg (cstr)
      C.free (cstr)
      battle_change_st (ss, sbst, btoi(sisq), sbtime, sfst)
      battle_change_st (1 - ss, obst, btoi(oisq), obtime, ofst)
}

func (uv * GtkUserView)     BattleBurn (side int, st * mbg.BattleState) {// 战斗中被烧
      var msg, sss string
      if side == 0 {sss = "攻方"} else {sss = "守方"}
...
ui_show_func func (name *byte, role *byte, job int, loc int, hpmax float32, hp float32, prop *float32, is_quit int, lst int, pst int),
) {
      ...
      cs1 := C.CString (pname)
      cs2 := C.CString (rname)
      ui_show_func (cs1, cs2, job, loc, hpmax, hp, &(prop[0]), btoi(is_quit), lst, pst)
      C.free (cs1)
      C.free (cs2)
}

func ShowCityInfo (d * mbg.Driver, cind int,
ui_show_func func (name *byte, scope int, hpmax float32, hp float32, role *byte, nmos int, mayor *byte, treasurer *byte, fengshui float32),
) {
      ...
      ui_show_func (cs1, scope, hpmax, hp, cs2, len(mos), cs3, cs4, fengshui)
      C.free (cs1)
      C.free (cs2)
      C.free (cs3)
      C.free (cs4)
...
ui_show_tech func (name *byte, study float32, scond int),
) {
      tname, study, scond, _, _, _, _, _ := mbg.GetTechInfo (hind)
      cstr := C.CString(tname)
      ui_show_tech (cstr, study, scond)
      C.free (cstr)
}
...
func (oi * GtkOperationInterface) Check () {
      ...
      case 0:
            ...
            for i := 0; i < nrole; i ++ {
                  ...
                  cstr := C.CString(name)
                  add_role_list (cstr, loc, money, dir, mst, mtime, btoi(cst), ast, atime)
                  C.free (cstr)
            }
            end_role_list()
      ...
      case 4:
            ...
            for i := 0; i < ntrain; i ++ {
                  ...
                  add_train_list (cs1, int(item), cs2, round)
                  C.free (cs1)
                  C.free (cs2)
            }
            end_train_list ()
...
func add_saloon_list_functor (price float32) (
      res_func func (name *byte, role *byte, job int, loc int, hpmax float32, hp float32, prop *float32, is_quit int, lst int, pst int),
) {
      return func (name *byte, role *byte, job int, loc int, hpmax float32, hp float32, prop *float32, is_quit int, lst int, pst int) {
...
func (oi * GtkOperationInterface)     SelCardObjCity (card_id int) int {// 返回卡片作用对象城市. -1 取消
      ...
      for {
            ...
            if obj_type == -1 {return -1}
            if obj_type == -3 {
                  oi.Check()
                  continue
            }
            if obj_type != -2 {continue}
            loc := get_obj()
...
func make_add_exchange_func (side int) (func (name *byte, role *byte, job int, loc int, hpmax float32, hp float32, prop *float32, is_quit int, lst int, pst int)) {
      return func (name *byte, role *byte, job int, loc int, hpmax float32, hp float32, prop *float32, is_quit int, lst int, pst int) {
...
func (oi * GtkOperationInterface) ExchangeCityPeople (cind int, splist []int, oplist []int) (in_list []int, out_list []int) {// 调度随身人员与目标内驻扎人员. 
      ...
      for k := 0; k < 2; k ++ {
            cstr := C.CString(msg)
            begin_exchange_people_list (k, cstr)
            C.free (cstr)
            var plist []int
            if k == 0 {plist = splist} else {plist = oplist}
            for _, i := range plist {
